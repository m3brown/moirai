...
(function() {
  ...
  clusters.getInstancesToShutdown = function(client) {
    var params;
    params = {
      endKey: +new Date()
    };
    return client.use('moirai').view('moirai', 'cluster_scheduled_shutdown', params, 'promise').then(function(clusters) {
      var awsIds;
      awsIds = _.chain(clusters.rows).pluck('value').flatten(true).compact().value();
      return Promise.resolve(awsIds);
    });
  };

  clusters.getCreateObject = function(date) {
    var DAY_IN_MILLIS, createdDateTime, scheduledDateTime;
