...
(function() {
  ...
  PRIVATE_KEY = conf.AWS.PRIVATE_KEY_FILE || (process.env.HOME + '/.ssh/id_rsa');

  USERNAME = conf.AWS.SSH_USER || 'ec2-user';

  DEFAULT_KEYS = conf.AWS.AUTHORIZED_KEY_DEFAULTS || [];
  ...
  getSSHCommand = function(host, pubkeys) {
    ...
    return cmd = 'ssh -o StrictHostKeyChecking=no -i ' + PRIVATE_KEY + ' ' + USERNAME + '@' + host + ' ' + remoteCommand;
  };
  ...
  keys.setSSHKeys = function(instance, pubkeys) {
    return ec2Client.getSingleInstance(instance.aws_id).then(function(data) {
      ...
      if (initialState === 'running' || initialState === 'pending') {
        ...
        return ec2Client.startInstance(instance.aws_id).then(makeAttempt).then(function() {
          return ec2Client.stopInstance(instance.aws_id);
        });
      }
    });
  };
