...
(function() {
  ...
  ec2PromiseClient = function(ec2_opts) {
    ...
    client = {};
    ['createTags', 'describeInstances', 'runInstances', 'terminateInstances', 'startInstances', 'stopInstances'].forEach(function(method) {
  ...
  instances.generateParams = function(opts) {
    ...
    userParams = _.pick.apply(_, [opts, 'ClientToken'].concat(slice.call(conf.AWS.ALLOWED_USER_PARAMS)));
  ...
  instances.startInstances = function(awsIds) {
    var params;
    if (!_.isArray(awsIds)) {
    ...
    return instances.ec2.startInstances(params);
  };

  instances.stopInstances = function(awsIds) {
    var params;
    if (!_.isArray(awsIds)) {
      return Promise.reject('Object is not an array: ' + JSON.stringify(awsIds));
    }
    ...
    return instances.ec2.stopInstances(params);
  };
  ...
  instances.destroyInstance = function(aws_id) {
    ...
    return instances.ec2.terminateInstances(params)["catch"](function(err) {
